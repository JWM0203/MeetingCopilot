...
- 🎧 **直接听对方的声音，不进会**：采集 Windows 系统回环音频，任何会议软件（腾讯会议 / Zoom / Teams / 飞书 …）都适用，无需机器人入会。麦克风为独立通道，可单独转录你自己的发言。
...
- 🥷 **隐身**：内容保护让窗口在 OBS / 屏幕共享 / 截图中完全不可见，全局快捷键一键隐藏呼出。
...
| 本地 Whisper turbo | ~2 s | 免费 | ✅ 完全本地 | 离线兜底；DirectML GPU 编码器 |
...
配好即用——应用会**自动拉起并回收**引擎（`tools/funasr_stream_server.py`，`ws://127.0.0.1:10097`）。模型首次运行时从 ModelScope 自动下载（约 1–2 GB）。Python 装在别处时，设置环境变量 `MC_FUNASR_PYTHON` 指向完整路径即可。
...
- API key 用 Windows DPAPI（`safeStorage`）加密落盘，永远不进渲染进程。
- 全部数据（设置 / 会话 / 资料）存在 `%APPDATA%/MeetingCopilot/` 的本地 JSON 里。无遥测、无账号、无服务器。
